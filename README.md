...
- API: `GET /api/explore?q=&software=&verified=1&online=1` (público), `GET /api/explore/<id>/icon` (público), `POST /api/explore/<id>/report`, `POST /api/servers/<id>/explore` (dono), `GET`/`POST /api/admin/explore` (administrador), `GET`/`POST /api/support`, `POST /api/support/<id>/reply|close`, `GET /api/admin/support` e `POST /api/admin/support/reply` (administrador). Testes: `test_explore.py` (84 verificações).
